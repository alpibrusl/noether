...
| M3 | Optimizer + Richer Types | ⏳ In progress | targeting v0.8.0 | Graph optimizer (framework + `dead_branch` + `canonical_structural` landed; `fuse_pure_sequential` / `hoist_invariant` / `memoize_pure` require deeper design — executor-level rather than AST), parametric polymorphism on stage signatures, row polymorphism on records, refinement types with runtime check |
