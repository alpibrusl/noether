...
| M3 | Optimizer + Richer Types | ⏳ In progress | targeting v0.8.0 | Graph optimizer: framework + `dead_branch` + `canonical_structural` + `memoize_pure` ✅ landed; `fuse_pure_sequential` and `hoist_invariant` remain (planner/executor work, not AST). Parametric polymorphism: Robinson-style unification module ✅ landed (on its own minimal `Ty`); NType integration + generic stdlib stages pending. Row polymorphism and refinement types still pending. |
